...
const Person = ({photo, name, title, quote, superlative, deleteStudent, id}) => {
    ...
    return (
        <div className="Person">
            ...
            <h3 contentEditable={isEditable}>{name === "" ? "Add Name Here" : name}</h3>
            <p contentEditable={isEditable}><em>{quote === "" ? "Add Quote Here" : `"${quote}"`}</em></p>
            <h3 contentEditable={isEditable}>{superlative === "" ? "Add Superlative Here" : superlative}</h3>
